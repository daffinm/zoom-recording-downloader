...
from meeting_metadata import MetadataDB, CsvFile
...
class TestMetadataDB(unittest.TestCase):
    ...
    zoom_meeting_data_is_listed = zoom_meeting_data_to_download
    zoom_meeting_data_is_not_listed = {'account_id': 'NKeXSYsCQumvGVXQMqbwRA', 'duration': 51, 'host_id': 'JI5dmsyfS2Sa06vQmSAH3Q', 'id': 6402292580, 'recording_count': 3, 'recording_files': [{'download_url': 'https://us02web.zoom.us/rec/download/wV6DY-Z1ptgFbRDjGEkW6ZJ6-XeVEpE6Gn_MF_T4o13xYrPf_7m82KmgeCQ4J91V0BKexEb97ug_5uwF.GnM4lD2HLXHG-PPz', 'file_extension': 'M4A', 'file_size': 46791654, 'file_type': 'M4A', 'id': '83d7850a-6342-41dd-a0d8-1e67e0005c0d', 'meeting_id': 's2bfT8qVQu+nV+zQ0/u33w==', 'play_url': 'https://us02web.zoom.us/rec/play/wV6DY-Z1ptgFbRDjGEkW6ZJ6-XeVEpE6Gn_MF_T4o13xYrPf_7m82KmgeCQ4J91V0BKexEb97ug_5uwF.GnM4lD2HLXHG-PPz', 'recording_end': '2023-01-13T02:50:43Z', 'recording_start': '2023-01-13T01:59:41Z', 'recording_type': 'audio_only', 'status': 'completed'}, {'download_url': 'https://us02web.zoom.us/rec/download/kGj95h3hodn9Y6I2oCNItVDD-RaVEIUDttit7ujbVaYN7IfnwkJwW4So74-euMuxx0frgYBi7xRlEPc.xWyExx3VypZhH-mL', 'file_extension': 'TXT', 'file_size': 45, 'file_type': 'CHAT', 'id': 'b6865a50-61e3-4b20-b115-f2cc3e4c3515', 'meeting_id': 's2bfT8qVQu+nV+zQ0/u33w==', 'play_url': 'https://us02web.zoom.us/rec/play/kGj95h3hodn9Y6I2oCNItVDD-RaVEIUDttit7ujbVaYN7IfnwkJwW4So74-euMuxx0frgYBi7xRlEPc.xWyExx3VypZhH-mL', 'recording_end': '2023-01-13T02:50:43Z', 'recording_start': '2023-01-13T01:59:41Z', 'recording_type': 'chat_file', 'status': 'completed'}, {'download_url': 'https://us02web.zoom.us/rec/download/q9xgnJkkN15NoKgEkwI5_88oya1swILIUQVBRz5vDWih1j5vA0ovtesQgTig0k59rcqkoKd8bA8r5WtL.LEbd2v99BOM99rCC', 'file_extension': 'MP4', 'file_size': 450812864, 'file_type': 'MP4', 'id': 'd200f35a-bc80-4c68-b179-ca8cd879d992', 'meeting_id': 's2bfT8qVQu+nV+zQ0/u33w==', 'play_url': 'https://us02web.zoom.us/rec/play/q9xgnJkkN15NoKgEkwI5_88oya1swILIUQVBRz5vDWih1j5vA0ovtesQgTig0k59rcqkoKd8bA8r5WtL.LEbd2v99BOM99rCC', 'recording_end': '2023-01-13T02:50:43Z', 'recording_start': '2023-01-13T01:59:41Z', 'recording_type': 'shared_screen_with_gallery_view', 'status': 'completed'}], 'recording_play_passcode': '', 'share_url': 'https://us02web.zoom.us/rec/share/8PHX7ALsxH3CVxfGD2p1PPrFuU_g7Ao5DG8nznLhkp78B3awLTGtAXyQKxHJraAZ.UtUkK8YUajloAtC1', 'start_time': '2023-01-13T01:59:40Z', 'timezone': 'America/Los_Angeles', 'topic': "David Wood's Personal Meeting Room", 'total_size': 497604563, 'type': 4, 'uuid': 's2bfT8qVQu+nV+zQ0/u33w=='}
    ...
    # ------------------------------------------------------------------------------------------------------------------
    # Tests for MetadataDB functionality -------------------------------------------------------------------------------
    # ------------------------------------------------------------------------------------------------------------------
    ...
    def test_open_search_and_save_does_not_change_the_file_contents(self):
        ...
        self.assertTrue(not result.empty, "Meeting data should be found in the metadata")
        self.db.save()
        self.assert_files_equal(filepath_expected=self.filepath_csv_original, filepath_actual=self.filepath_csv_test)
    ...
    def test_open_update_and_save_changes_the_file_contents(self):
        ...
        self.db.save()
        difference_in_bytes = len(CsvFile.Values.YES.encode('utf-8'))
        with open(self.filepath_csv_original, 'rb') as original_file, open(self.filepath_csv_test, 'rb') as test_file:
            original_size = len(original_file.read())
            test_size = len(test_file.read())
            self.assertEqual(test_size, original_size + difference_in_bytes, "File size should increase by the size of the 'downloaded' status")

    # def test_meeting_listed_in_metadata(self):
    #     meeting_data = self.zoom_meeting_data_is_listed
    #     result = self.db.is_meeting_listed(meeting_data)
    #     self.assertTrue(result, "Meeting data should be found in the metadata")

    # def test_meeting_not_listed_in_metadata(self):
    #     meeting_data = self.zoom_meeting_data_is_not_listed
    #     result = self.db.is_meeting_listed(meeting_data)
    #     self.assertFalse(result, "Meeting data should NOT be listed in the metadata")
    ...
    def test_meeting_is_to_be_deleted_true(self):
        ...
        self.assertTrue(not result1.empty, "Meeting data should be found in the metadata")
        result2 = self.db.should_ignore_meeting(meeting_data)
